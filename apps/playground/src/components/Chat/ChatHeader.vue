...
<template>
  <div class="flex items-center gap-3 bg-ws-ui-bg text-ws-ui-text p-3 rounded-t-lg w-full">
    <span class="block w-10 h-10 rounded-full">
      <svg
        ...
        width="212"
        class="w-full h-full"
      ><title>default-user</title><path
        class="text-ws-avatar-bg"
        fill="currentColor"
    ...
    <h3 class="flex-1">
      + {{ config.phoneNumberTo || "Number to test" }}
    </h3>
    <span
      class="w-6 h-6 text-ws-mute cursor-pointer"
    ><svg
      viewBox="0 0 24 24"
      height="24"
      width="24"
